...
# Base Articles
Article.create!(title: 'ENSL Developer', status: Article::STATUS_PUBLISHED, category_id: Category.first.id,
                text: 'Welcome to ENSL', user: User.first)
Article.create!(id: Article::RULES, title: 'Rules', status: Article::STATUS_PUBLISHED,
                category_id: Category.where(name: 'ENSL Rules').first.id, text: 'ENSL Rules', user: User.first)
Article.create!(id: Article::HISTORY, title: 'History', status: Article::STATUS_PUBLISHED,
                category_id: Category.where(name: 'ENSL Rules').first.id, text: 'ENSL History', user: User.first)
Article.create!(id: Article::HOF, title: 'Hall of Fame', status: Article::STATUS_PUBLISHED,
                category_id: Category.where(name: 'ENSL Rules').first.id, text: 'Hall of Fame', user: User.first)
...
# Base Forums
Forum.create!(title: 'General Discussion', description: "Anything that doesn't belong in the other forums",
              category: Category.where(domain: Category::DOMAIN_FORUMS).first)

# Example Topic
Topic.create!(title: 'Hello World!', forum_id: Forum.first.id, user: User.first, first_post: 'Hello World!')

# Example Team
Team.create!(name: 'ENSL', irc: '#ensl', web: 'http://ensl.org', tag: '[ENSL]', country: 'EU', comment: 'ENSL Team',
             founder: User.first)
