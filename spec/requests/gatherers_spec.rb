...
RSpec.describe 'GatherersController', type: :request do
  ...
  describe 'POST /gatherers' do
    it 'redirects after a successful html join' do
      ...
      result = instance_double(Gathers::Result, success?: true, gatherer: gatherer, gather: gather, error: nil)
    ...
    it 'uses the gather from the returned gatherer in turbo-stream responses' do
      login_as(user)
      gatherer = create(:gatherer, gather: gather, user: user)
      result = instance_double(Gathers::Result, success?: true, gatherer: gatherer, gather: nil, error: nil)
      allow(Gathers::Join).to receive(:call).and_return(result)
  ...
  describe 'DELETE /gatherers/:id' do
    ...
    it 'uses the leave service and reports failures for self-removal' do
      login_as(user)
      result = instance_double(Gathers::Result, success?: false, gather: gather, error: 'Cannot leave')
