...
RSpec.feature 'Forums Management', :js, type: :feature do
  ...
  feature 'Editing forums' do
    scenario 'admin can access forum edit page' do
      ...
      expect(page).to have_text('Access Rights')
    end
    ...
    scenario 'admin can update forum title and description' do
      ...
      expect(page).to have_text(I18n.t('flash.actions.update.notice', resource_name: Forum.model_name.human))
      expect(page).to have_text('Updated Forum Title')
    end
  end
  ...
  feature 'Managing forum access rights' do
    ...
    scenario 'admin can update existing group access level' do
      ...
      expect(page).to have_text(I18n.t('groups.acl.update'))
    ...
    scenario 'access level dropdown displays all available options' do
      ...
      within('.add-acl') do
        access_options = find_by_id('forumer_access').all('option').map(&:text)
