...
feature 'Issues', :js do
  ...
  scenario 'creation' do
    ...
    click_button 'Submit'
    expect(page).to have_text('Issue was successfully created.')
    expect(page).to have_text(issue_title)
    expect(page).to have_text(issue_description)
  end
  ...
  feature 'adminstration' do
    ...
    scenario 'issue management' do
      ...
      within '#open' do
        ...
        expect(page).to have_text(issue.author.username)
      end
      visit edit_issue_path(issue)
      expect(page).to have_text('Editing Issue')
