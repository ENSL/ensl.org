...
RSpec.describe Group, type: :model do
  ...
  describe '#to_s' do
    it 'returns the group name' do
      g = described_class.new(name: 'TeamX')
  ...
  describe 'class helpers for group member retrieval' do
    before do
      ...
      @admin_group = described_class.find_or_create_by(id: Group::ADMINS) { |g| g.name = 'Admins' }
      @caster_group = described_class.find_or_create_by(id: Group::CASTERS) { |g| g.name = 'Casters' }
      @ref_group = described_class.find_or_create_by(id: Group::REFEREES) { |g| g.name = 'Referees' }
    ...
    describe '.referees' do
      it 'returns referee groupers' do
        referees = described_class.referees
      ...
      it 'returns empty array if no referees exist' do
        ...
        expect(described_class.referees).to be_empty
      end
    end

    describe '.casters' do
      it 'returns caster groupers' do
        casters = described_class.casters
      ...
      it 'returns empty array if no casters exist' do
        ...
        expect(described_class.casters).to be_empty
      end
    end

    describe '.staff' do
      it 'aggregates staff from multiple groups' do
        staff_users = described_class.staff.map(&:user)
      ...
      it 'returns empty array when no staff exist' do
        Grouper.destroy_all
        expect(described_class.staff).to be_empty
      end
      ...
      it 'handles groupers with nil user_id in staff groups gracefully' do
        ...
        expect { described_class.staff }.not_to raise_error
        expect(described_class.staff.map(&:user_id)).not_to include(nil)
      end
      ...
      it 'handles groupers with non-existent user_id in staff groups gracefully' do
        ...
        # Should not raise an exception - valid_users scope filters these out
        expect { described_class.staff }.not_to raise_error
        expect(described_class.staff.map(&:user_id)).not_to include(99_999)
      end
    ...
    describe '.extras' do
      ...
      it 'handles groupers with nil user_id gracefully without breaking' do
        ...
        # Should not raise an exception - valid_users scope filters these out
        expect { described_class.extras }.not_to raise_error
        # Extras should be empty or not include the nil user_id
        expect(described_class.extras.map(&:user_id)).not_to include(nil)
      end
      ...
      it 'handles groupers with non-existent user_id gracefully' do
        ...
        expect(described_class.extras.map(&:user_id)).not_to include(99_999)
      end
    ...
    describe '.gathermods' do
      it 'returns gather moderator groupers' do
        gathermod_group = described_class.find_or_create_by(id: Group::GATHER_MODERATORS) { |g| g.name = 'Gather Mods' }
        gathermod_user = create(:user)
        create(:grouper, group: gathermod_group, user: gathermod_user)

        gathermods = described_class.gathermods
    ...
    describe '.contributors' do
      it 'returns contributor groupers' do
        contrib_group = described_class.find_or_create_by(id: Group::CONTRIBUTORS) { |g| g.name = 'Contributors' }
        contrib_user = create(:user)
        create(:grouper, group: contrib_group, user: contrib_user)

        contributors = described_class.contributors
    ...
    describe '.predictors' do
      it 'returns predictor groupers' do
        ...
        predictors = described_class.predictors
      ...
      it 'handles groupers with invalid user_ids gracefully' do
        ...
        # Should not raise an exception - valid_users scope filters these out
        expect { described_class.predictors }.not_to raise_error
      end
