...
RSpec.describe Comment, type: :model do
  ...
  describe 'validations' do
    it 'is valid with a user, commentable and text' do
      c = described_class.new(user: user, commentable: post_record, text: 'hello')
      expect(c).to be_valid
    end

    it 'requires a user' do
      c = described_class.new(commentable: post_record, text: 'x')
      expect(c).not_to be_valid
      expect(c.errors[:user]).to be_present
    end

    it 'requires a commentable' do
      c = described_class.new(user: user, text: 'x')
      expect(c).not_to be_valid
    ...
    it 'validates text length (max 10000)' do
      long = 'a' * 10_001
      c = described_class.new(user: user, commentable: post_record, text: long)
      expect(c).not_to be_valid
  ...
  describe 'callbacks' do
    it 'leaves text_parsed unchanged when text is nil' do
      c = described_class.new(user: user, commentable: post_record, text: nil)

      expect { c.parse_text }.not_to change(c, :text_parsed)
    end

    it 'parses text into text_parsed before save' do
      c = described_class.new(user: user, commentable: post_record, text: '[b]bold[/b]')
  ...
  describe 'permissions helpers' do
    describe '#can_create?' do
      it 'returns false when no current user provided' do
        c = described_class.new(user: user, commentable: post_record, text: 'x')
    ...
    describe '#can_update?' do
      it 'allows the owner to update' do
        c = described_class.create!(user: user, commentable: post_record, text: 'x')
  ...
  describe '.params' do
    ...
    it 'raises when comment key is missing' do
      params = ActionController::Parameters.new({})
      expect { described_class.params(params, nil) }.to raise_error(ActionController::ParameterMissing)
    end
  end
