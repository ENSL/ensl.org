...
RSpec.describe UsersController, type: :controller do
  ...
  let!(:invalid_params) { params.merge(steamid: (50..150).map { rand(65..90).chr }.join) }
  let!(:invalid_email_params) { params.merge(email: 'not-an-email') }
  let!(:blank_email_params) { params.merge(email: '') }
  ...
  describe 'GET #new' do
    it 'redirects logged-in users to edit' do
      login user.username
      get :new
      expect(response).to redirect_to(edit_user_path(user))
    end

    it 'allows admin to access new and does not remove their profile' do
      # admin is defined above as a let! helper
      login_admin
      expect(admin.profile).to be_present
      profile_id = admin.profile.id
      get :new
      expect(response).to render_template('new')
      expect(Profile.exists?(profile_id)).to be true
    end
  end
  ...
  context 'POST' do
    ...
    describe 'with invalid values' do
      ...
      it 'rejects blank email' do
        login_admin
        count = User.count
        post :create, params: { user: blank_email_params }
        expect(User.count).to eq(count)
        expect(response).to render_template('new')
      end
    end
