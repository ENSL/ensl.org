...
RSpec.describe GathersHelper, type: :helper do
  ...
  describe '#render_gather' do
    before do
      allow(helper).to receive_messages(headers: {}, render: 'rendered')
    end
    ...
    it 'marks voting gathers as voted when the current user has voted' do
      votes_relation = double('VotesRelation')
      ...
      helper.define_singleton_method(:cuser) { current_user }
      allow(votes_relation).to receive(:where).with(user_id: 9).and_return(double('FilteredVotes', any?: true))
    ...
    it 'marks voting gathers as voting when the current user has not voted' do
      ...
      allow(helper).to receive_messages(gather_from_context: gather, gatherer_from_context: nil)
      helper.define_singleton_method(:cuser) { current_user }
      allow(votes_relation).to receive(:where).with(user_id: 9).and_return(double('FilteredVotes', any?: false))
  ...
  describe '#gather_music_should_play?' do
    ...
    let(:gather_users) { double('GatherUsers') }
    let(:gatherer_votes) { double('VoteScope') }
    let(:map_votes) { double('VoteScope') }
    let(:server_votes) { double('VoteScope') }
    let(:gather) do
      instance_double(
        Gather,
        users: gather_users,
    ...
    before do
      allow(helper).to receive_messages(gather_from_context: gather, gather_current_user: user)
    end
    ...
    it 'returns false when any vote has already been cast' do
      allow(gather_users).to receive(:exists?).with(5).and_return(true)
      allow(gatherer_votes).to receive(:where).with(user_id: 5).and_return(double('GathererVoteQuery', exists?: true))
    ...
    it 'returns true when the user can still vote on all categories' do
      allow(gather_users).to receive(:exists?).with(5).and_return(true)
      allow(gatherer_votes).to receive(:where).with(user_id: 5).and_return(double('GathererVoteQuery', exists?: false))
      allow(map_votes).to receive(:where).with(user_id: 5).and_return(double('MapVoteQuery', exists?: false))
      allow(server_votes).to receive(:where).with(user_id: 5).and_return(double('ServerVoteQuery', exists?: false))
