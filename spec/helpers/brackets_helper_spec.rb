...
RSpec.describe BracketsHelper, type: :helper do
  ...
  describe '#render_connector?' do
    let(:relation) { double('BracketerRelation') }
    ...
    it 'returns true when both adjacent cells exist and are enabled' do
      above_scope = double('BracketerScope', first: instance_double(Bracketer, disabled: false))
      below_scope = double('BracketerScope', first: instance_double(Bracketer, disabled: false))
    ...
    it 'returns false when one adjacent cell is disabled' do
      above_scope = double('BracketerScope', first: instance_double(Bracketer, disabled: true))
      below_scope = double('BracketerScope', first: instance_double(Bracketer, disabled: false))
